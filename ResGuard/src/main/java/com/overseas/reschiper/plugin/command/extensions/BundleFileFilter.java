package com.overseas.reschiper.plugin.command.extensions;
...
import com.overseas.reschiper.plugin.operations.NativeLibrariesOperation;
...
public class BundleFileFilter {
    ...
    private Files.NativeLibraries updateLibDirectory(@NotNull BundleModule bundleModule, @NotNull List<ModuleEntry> entries) throws UnexpectedException {
        ...
        for (Files.TargetedNativeDirectory directory : nativeLibraries.getDirectoryList()) {
            int directoryNativeSize = (int) libEntries.stream()
                    .filter(entry -> entry.getPath().startsWith(directory.getPath())).count();
            if (directoryNativeSize > 0) {
                int moduleNativeSize = (int) bundleModule.getEntries().stream()
                        .filter(entry -> entry.getPath().startsWith(directory.getPath())).count();
