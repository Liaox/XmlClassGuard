...
public class StringObfuscator {
    ...
    public void reset(HashSet<Pattern> blacklistPatterns) {
        ...
        isReplaced.clear();
        isWhiteList.clear();
        List<String> a_z = Arrays.asList(A_TO_Z);
        Collections.shuffle(a_z);
        List<String> a_all = Arrays.asList(A_TO_ALL);
        Collections.shuffle(a_all);
        for (String str : a_z)
            if (Utils.match(str, blacklistPatterns))
                replaceStringBuffer.add(str);

        for (String first : a_z)
            for (String aMAToAll : a_all) {
        ...
        for (String first : a_z)
            for (String second : a_all)
                for (String third : a_all) {
